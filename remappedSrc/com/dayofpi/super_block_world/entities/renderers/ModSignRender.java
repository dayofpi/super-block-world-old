...
public class ModSignRender implements BlockEntityRenderer<ModSignBE> {
    private static final int RENDER_DISTANCE = MathHelper.square(16);
    private final Map<SignType, ModSignRender.SignModel> typeToModel;
    private final TextRenderer textRenderer;

    public ModSignRender(BlockEntityRendererFactory.Context ctx) {
    ...
    public void render(ModSignBE entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        if (blockState.getBlock() instanceof ModSign) {
            ...
            matrices.translate(0.5D, 0.5D, 0.5D);
            h = -blockState.get(ModWallSign.FACING).asRotation();
            matrices.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(h));
    ...
    public static SignType getSignType(Block block) {
        SignType signType2;
        if (block instanceof ModSignTemplate) {

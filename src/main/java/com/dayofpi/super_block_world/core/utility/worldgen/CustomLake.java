...
public class CustomLake extends Feature<SingleStateFeatureConfig> {
    ...
    public CustomLake(Codec<SingleStateFeatureConfig> codec) {
        super(codec);
    }
                        ...
                        for (ac = 4; ac < 8; ++ac) {
                            if (bls[(ae * 16 + ab) * 8 + ac]) {
                                ...
                                if (isSoil(structureWorldAccess.getBlockState(blockPos5)) && structureWorldAccess.getLightLevel(LightType.SKY, blockPos.add(ae, ac, ab)) > 0) {
                                    ...
                                    } else if (biome.getGenerationSettings().getSurfaceConfig().getUnderwaterMaterial().isOf(BlockReg.TOADSTOOL_SOIL)) {
                                        structureWorldAccess.setBlockState(blockPos5, BlockReg.TOADSTOOL_FARMLAND.getDefaultState(), Block.NOTIFY_LISTENERS);
                                    } else {
                                        structureWorldAccess.setBlockState(blockPos5, BlockReg.TOADSTOOL_GRASS.getDefaultState(), Block.NOTIFY_LISTENERS);
                                    }
                                }
                            }
                        }

...
public abstract class PoisonFluid extends FluidTemplate {
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        if (direction == Direction.DOWN) {
            ...
            if (this.isIn(Tags.POISON) && fluidState2.isIn(FluidTags.LAVA)) {
                if (state.getBlock() instanceof FluidBlock) {
                    world.setBlockState(pos, BlockTypes.VANILLATE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public Item getBucketItem() {
        return ItemTypes.POISON_BUCKET;
    }
    ...
    protected BlockState toBlockState(FluidState fluidState) {
        return BlockTypes.POISON.getDefaultState().with(FluidBlock.LEVEL, getBlockStateLevel(fluidState));
    }
